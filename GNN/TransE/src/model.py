...
class TransE:
    ...
    def infer(self, triple_pos, triple_neg):
        with tf.name_scope('lookup'):
            head_pos = tf.nn.embedding_lookup(self.entity_embedding, triple_pos[:, 1])
            tail_pos = tf.nn.embedding_lookup(self.entity_embedding, triple_pos[:, 2])
            relation_pos = tf.nn.embedding_lookup(self.relation_embedding, triple_pos[:, 0])
            head_neg = tf.nn.embedding_lookup(self.entity_embedding, triple_neg[:, 1])
            tail_neg = tf.nn.embedding_lookup(self.entity_embedding, triple_neg[:, 2])
            relation_neg = tf.nn.embedding_lookup(self.relation_embedding, triple_neg[:, 0])
    ...
    def evaluate(self, eval_triple):
        with tf.name_scope('lookup'):
            head = tf.nn.embedding_lookup(self.entity_embedding, eval_triple[1])
            tail = tf.nn.embedding_lookup(self.entity_embedding, eval_triple[2])
            relation = tf.nn.embedding_lookup(self.relation_embedding, eval_triple[0])
        with tf.name_scope('link'):
